...
class Blockchain {
    ...
    _addBlock(block) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            // only add previous block hash after genesis block added
            ...
            let validationError = await self.validateChain();
            //if no errors in blockchain
            if (validationError.length === 0) {
                //add the new block
                self.chain.push(block);
                //update height
                self.height++;
                // return new block via promise 
                resolve(block);
            } else {
                // reject new block
    ...
    requestMessageOwnershipVerification(address) {
        return new Promise((resolve) => {
            ...
            //const star = "starRegistry";
            //const ownershipMessage = address+":"+time+":"+star;
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if (currentTime - msgTime < fiveMinInSeconds) {
                if (bitcoinMessage.verify(message, address, signature)) {
                    ...
                    //creation of the new block with the owner and the star 
                    let block = new BlockClass.Block({ "owner": address, "star": star });

                    //Add the block
                    self._addBlock(block);

                    //Resolve with the new block
                    resolve(block);
    ...
    getStarsByWalletAddress(address) {
        ...
        let stars = [];
        return new Promise((resolve) => {
            self.chain.forEach(async (starBlock) => {

                // decoded data
                let data = await starBlock.getBData();
    ...
    validateChain() {
        ...
        return new Promise((resolve) => {
            ...
            // Iterate blocks, compare hashes
            let validatePromises = [];
            self.chain.forEach((block, index) => {
                if (block.height > 0) {
                    const previousBlock = self.chain[index - 1];
                    if (block.previousBlockHash !== previousBlock.hash) {
                        const errorMessage = `Block ${index} previous: ${block.previousBlockHash}, actual: ${previousBlock.hash}`;
                        errorLog.push(errorMessage);
                    }
                }

                // Push promise for validation
                ...
                .then(validatedBlocks => {
                    validatedBlocks.forEach((valid, index) => {
                        if (!valid) {
                            ...
                            const errorMessage = `Block ${index} invalid hash: (${invalidBlock.hash})`;
                            errorLog.push(errorMessage);
                        }
                    });
